<?php if (is_page(5)) { ?>
	<link rel="stylesheet" href="<?php echo get_stylesheet_directory_uri(); ?>/assets/css/new-style.css" />
	<script type="text/javascript">
		function msieversion() {
			var ua = window.navigator.userAgent;
			var msie = ua.indexOf("MSIE ");
			if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
				jQuery('.ie').addClass('show')
				document.body.style.overflowY = "hidden";
				document.body.style.overflowX = "hidden";
			}
			return false;
		}

			jQuery(document).ready(function() {

				msieversion();
				jQuery("#mainMenu").append('<span class="pink"></span>')

				jQuery("ul.menu-list > li:first-child").mouseenter(function() {
					jQuery("#mainMenu > span").removeClass();
					jQuery("#mainMenu > span").addClass('pink')
				});

				jQuery("ul.menu-list > li:nth-child(2), ul.menu-list > li:nth-child(5)").mouseenter(function() {
					jQuery("#mainMenu > span").removeClass();
					jQuery("#mainMenu > span").addClass('blue')
				});

				jQuery("ul.menu-list > li:nth-child(3), ul.menu-list > li:nth-child(6)").mouseenter(function() {
					jQuery("#mainMenu > span").removeClass();
					jQuery("#mainMenu > span").addClass('orange')
				});

				jQuery("ul.menu-list > li:nth-child(4)").mouseenter(function() {
					jQuery("#mainMenu > span").removeClass();
					jQuery("#mainMenu > span").addClass('green')
				});

				jQuery("#main-menu, .menu-list").mouseleave(function() {
					jQuery("#mainMenu > span").removeClass();
					jQuery("#mainMenu > span").addClass('pink')
				});
			})		
	</script>
<?php } ?>
<?php
/**
 * Header-v1 template.
 *
 * @author     ThemeFusion
 * @copyright  (c) Copyright by ThemeFusion
 * @link       https://theme-fusion.com
 * @package    Avada
 * @subpackage Core
 */

// Do not allow directly accessing this file.
if (! defined('ABSPATH')) {
    exit('Direct script access denied.');
}
?>
<header>
	<nav>
		<button id="toggleMenu" type="button"></button>
		<a href="/" class="logo">
        	<img src='<?php echo get_stylesheet_directory_uri(); ?>/assets/images/menu-bar-logo.png' alt="">
        </a>
<?php /*?>        <a href="/" class="download-icon">
        	<img src='data:image/png;base64,' alt="">
        </a>
<?php */?>	</nav>
	
	<?php
        wp_nav_menu(array(
            'menu'   => 'Horizontal Menu',
            'menu_class' => 'menu-list',
            'link_before' => '<span class="no"></span>',
            'menu_id' => '',
            'container_id' => 'mainMenu',
            'container_class' => 'menu-wrapper'
        ));
    ?>
</header>


<script type="text/javascript">
	document.getElementById('toggleMenu').onclick = function() {
		this.classList.toggle('is-active');
		const menu = document.getElementById('mainMenu');
		const classes = menu.classList;
		classes.toggle('show');
	}
</script>